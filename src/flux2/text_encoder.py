...
logger = logging.getLogger("flux2_text_encoder")
...
def load_qwen3_embedder(variant: str, device: str | torch.device = "cuda"):
    ...
    local_only = os.environ.get("FLUX2_LOCAL_ONLY", "1").strip() == "1"
    local_candidates = [
        repo_root / "weights" / "text_encoder" / f"Qwen--Qwen3-{variant}-FP8",
        repo_root / "weights" / "text_encoder" / f"Qwen--Qwen3-{variant}",
        repo_root / "weights" / "text_encoder" / f"Qwen--Qwen3-{variant}-Instruct",
    ]

    candidates: list[str] = [str(path) for path in local_candidates if path.exists()]
    if not local_only:
        candidates.extend(
            [
                f"Qwen/Qwen3-{variant}-FP8",
                f"Qwen/Qwen3-{variant}",
                f"Qwen/Qwen3-{variant}-Instruct",
            ]
        )

    if not candidates:
        raise RuntimeError(
            f"No local Qwen3-{variant} text encoder bundle found under weights/text_encoder/ and local-only mode is enabled."
        )

    last_error: Exception | None = None
    for model_spec in candidates:
        try:
            logger.info("Attempting to load Qwen3 text encoder: %s", model_spec)
            return Qwen3Embedder(model_spec=model_spec, device=device)
        except ModuleNotFoundError as exc:
            last_error = exc
            if "triton" in str(exc).lower():
                logger.warning(
                    "Triton dependency is unavailable for %s; trying fallback model.",
                    model_spec,
                )
                continue
            raise
            ...
            last_error = exc
            logger.warning("Failed to load %s (%s). Trying fallback.", model_spec, exc)
            continue

    raise RuntimeError(
        "Unable to load any Qwen3 text encoder variant. "
        "Tried FP8 and non-FP8 model IDs."
    ) from last_error
