...
def denoise(
    ...
    progress_callback: Callable[[int, int, float, float, Tensor], None] | None = None,
):
    import logging
    logger = logging.getLogger("flux2_sampling")
    
    guidance_vec = torch.full((img.shape[0],), guidance, device=img.device, dtype=img.dtype)
    total_steps = max(1, len(timesteps) - 1)
    logger.info(f"denoise() starting: total_steps={total_steps}, timesteps={len(timesteps)}")
    
    for step_idx, (t_curr, t_prev) in enumerate(zip(timesteps[:-1], timesteps[1:]), start=1):
        logger.info(f"denoise step {step_idx}/{total_steps}: t_curr={t_curr:.3f}, t_prev={t_prev:.3f}")
        ...
        logger.info(f"denoise step {step_idx}: calling model.forward()")
        pred = model(
            x=img_input,
            x_ids=img_input_ids,
            timesteps=t_vec,
        ...
        logger.info(f"denoise step {step_idx}: model.forward() completed, pred.shape={pred.shape}")
        
        if img_input_ids is not None:
            pred = pred[:, : img.shape[1]]

        img = img + (t_prev - t_curr) * pred
        logger.info(f"denoise step {step_idx}: tensor update completed")

        if progress_callback is not None:
            progress_callback(step_idx, total_steps, float(t_curr), float(t_prev), img)
...
def denoise_cfg(
    ...
    logger.info(f"denoise_cfg() starting with guidance={guidance}")
    ...
    total_steps = max(1, len(timesteps) - 1)
    logger.info(f"denoise_cfg() total_steps={total_steps}")
    
    for step_idx, (t_curr, t_prev) in enumerate(zip(timesteps[:-1], timesteps[1:]), start=1):
        logger.info(f"denoise_cfg step {step_idx}/{total_steps}: t_curr={t_curr:.3f}, t_prev={t_prev:.3f}")
        ...
        logger.info(f"denoise_cfg step {step_idx}: calling model.forward()")
        pred = model(
            x=img_input,
            x_ids=img_input_ids,
            timesteps=t_vec,
        ...
        logger.info(f"denoise_cfg step {step_idx}: model.forward() completed, pred.shape={pred.shape}")
        ...
        img = img + (t_prev - t_curr) * pred
        logger.info(f"denoise_cfg step {step_idx}: tensor update completed")

        if progress_callback is not None:
            progress_callback(step_idx, total_steps, float(t_curr), float(t_prev), img.chunk(2)[0])
